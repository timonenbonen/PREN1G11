...
class CanvasInterface:
    def __init__(self, graph: Graph, canvas_size: float, possible_target_nodes: list[Cylinder], canvas_webots_relation):
    ...
    def draw_node(self, node):
        ...
        if node == self.graph.target_node:
            color = 'blue'
        elif node == self.graph.starting_node:
            color = 'green'
    ...
    def on_node_drag(self, event):
        if self.selected_node and self.graph.starting_node is not self.selected_node:
